...
impl WsListener {
    pub fn new(ws_url: String) -> (Self, mpsc::Receiver<PoolCreationEvent>) {
        ...
        (Self { ws_url, sender: tx, running: Arc::new(std::sync::atomic::AtomicBool::new(false)) }, rx)
    }

    pub async fn start(&self) -> SniperResult<()> {
        if self.running.load(std::sync::atomic::Ordering::Relaxed) {
            return Err(SniperError::Unknown { msg: "Already running".into() });
        }
        self.running.store(true, std::sync::atomic::Ordering::Relaxed);
        let url = self.ws_url.clone();
        let tx = self.sender.clone();
        let r = self.running.clone();
        tokio::spawn(async move { Self::loop_listen(&url, &tx, &r).await; });
        info!("WsListener started");
        Ok(())
    }

    pub fn stop(&self) { self.running.store(false, std::sync::atomic::Ordering::Relaxed); }

    async fn loop_listen(url: &str, tx: &mpsc::Sender<PoolCreationEvent>, running: &Arc<std::sync::atomic::AtomicBool>) {
        loop {
                                ...
                                if let Some(log) = futures::StreamExt::next(&mut stream).await {
                                    for line in &log.logs {
                                        if line.contains("Program log: Instruction: Create") {
                                            debug!("New pool: {:?}", log.signature);
                                            let _ = tx.send(PoolCreationEvent {
                                                mint: Pubkey::new_unique(), bonding_curve: Pubkey::new_unique(),
                                                associated_bonding_curve: Pubkey::new_unique(), user: Pubkey::new_unique(),
                                                name: "Token".into(), symbol: "TKN".into(), uri: "".into(), mayhem: false,
                                                slot: 0, timestamp: Utc::now(), signature: log.signature.clone(),
                                            }).await;
                                            break;
                                        }
                                    }
                                } else { warn!("Stream ended"); break; }
                            }
                        }
                        Err(e) => { error!("Subscribe fail: {}", e); tokio::time::sleep(std::time::Duration::from_secs(5)).await; }
                    }
                }
                Err(e) => { error!("Connect fail: {}", e); tokio::time::sleep(std::time::Duration::from_secs(5)).await; }
            }
        }
    }
}
