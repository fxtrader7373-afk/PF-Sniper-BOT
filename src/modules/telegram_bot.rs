...
impl TelegramBot {
    ...
    async fn dispatch(&self, msg: Message, cmd: Command) -> SniperResult<()> {
        ...
        if !self.is_auth(uid) {
            self.bot.send_message(msg.chat.id, "⛔").await?;
            return Ok(());
        }
        ...
        match cmd {
            Command::Start => {
                self.bot
                    .send_message(msg.chat.id, "🎯 pf-sniper v0.1.0")
                    .await?;
            }
            Command::Status => {
                let u = (Utc::now() - self.start_time).num_seconds();
                self.bot
                    .send_message(
                        msg.chat.id,
                        format!(
            ...
            Command::Balance => {
                self.bot.send_message(msg.chat.id, "💰 Paper mode").await?;
            }
            Command::Positions => {
                self.bot.send_message(msg.chat.id, "📈 None").await?;
            }
            Command::Pnl { period } => {
                ...
                self.bot
                    .send_message(
                        ...
                        format!(
                            "📈 {}:\nTrades: {}\nP&L: {:.4} SOL",
                            period, s.total_trades, s.total_pnl_sol
                        ),
                    )
                    .await?;
            }
            Command::Journal { n } => {
                ...
                let t = if trades.is_empty() {
                    "None".into()
                } else {
                    trades
                        .iter()
                        .take(5)
                        .map(|x| format!("{} {:.4} SOL", x.token_mint, x.net_pnl_sol))
                        .collect::<Vec<_>>()
                        .join("\n")
                };
                self.bot
            ...
            Command::Forcesell { token } => {
                ...
                self.bot
                    .send_message(msg.chat.id, format!("⚠️ {}", token))
                    .await?;
            }
            Command::Setwallet { label } => {
                info!("Wallet → {}", label);
                self.bot
                    .send_message(msg.chat.id, format!("🔑 {}", label))
                    .await?;
            }
            Command::Listwallets => {
                self.bot.send_message(msg.chat.id, "🔑 None").await?;
            ...
            Command::Setrpc { label, url } => {
                info!("RPC {}→{}", label, url);
                self.bot
                    .send_message(msg.chat.id, format!("🌐 {}→{}", label, url))
                    .await?;
            }
            Command::Setwss { url } => {
                info!("WSS→{}", url);
            ...
            Command::Setrisk { param, value } => {
                ...
                self.bot
                    .send_message(msg.chat.id, format!("⚙️ {}={}", param, value))
                    .await?;
            }
            ...
            Command::Retrain => {
                self.bot
                    .send_message(msg.chat.id, "🧠 Retraining...")
                    .await?;
            }
            Command::Abtest { action } => {
                self.bot
                    .send_message(msg.chat.id, format!("🔬 A/B {}", action))
                    .await?;
            }
            Command::Logs => {
                self.bot.send_message(msg.chat.id, "📜 Check logs").await?;
            }
            Command::Help => {
                self.bot
